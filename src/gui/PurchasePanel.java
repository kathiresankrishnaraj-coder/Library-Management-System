package gui;

import components.*;
import model.Purchase;
import model.Book;
import service.PurchaseManagement;
import service.BookManagement;
import utils.ExcelExporter;

import javax.swing.*;
import javax.swing.border.EmptyBorder;
import javax.swing.table.DefaultTableModel;
import java.awt.*;
import java.io.File;
import java.sql.Date;
import java.util.List;

public class PurchasePanel extends JPanel {

    private PurchaseManagement purchaseService = new PurchaseManagement();
    private BookManagement bookService = new BookManagement();

    private ModernTable table;
    private DefaultTableModel tableModel;

    public PurchasePanel() {
        setOpaque(false);
        setLayout(new BorderLayout(20, 20));
        setBorder(new EmptyBorder(20, 30, 20, 30));

        // Header Panel
        JPanel headerPanel = new JPanel(new BorderLayout());
        headerPanel.setOpaque(false);

        JLabel lblTitle = new JLabel("Supply Purchase & Acquisitions");
        lblTitle.setFont(new Font("Segoe UI", Font.BOLD, 20));
        lblTitle.setForeground(new Color(60, 60, 60));
        headerPanel.add(lblTitle, BorderLayout.WEST);

        // Actions
        JPanel actionsPanel = new JPanel(new FlowLayout(FlowLayout.RIGHT, 10, 0));
        actionsPanel.setOpaque(false);

        RoundedButton btnAdd = new RoundedButton("New Purchase", new Color(46, 204, 113), new Color(39, 174, 96));
        btnAdd.addActionListener(e -> showPurchaseDialog());

        RoundedButton btnExport = new RoundedButton("Export CSV", new Color(24, 144, 255), new Color(11, 101, 192));
        btnExport.addActionListener(e -> exportCSV());

        actionsPanel.add(btnExport);
        actionsPanel.add(btnAdd);
        headerPanel.add(actionsPanel, BorderLayout.EAST);
        add(headerPanel, BorderLayout.NORTH);

        // Table
        String[] columns = {"Purchase ID", "Book Title", "Supplier", "Quantity", "Unit Price", "Total Amount", "Purchase Date"};
        tableModel = new DefaultTableModel(columns, 0) {
            @Override
            public boolean isCellEditable(int row, int column) { return false; }
        };
        table = new ModernTable(tableModel);

        JScrollPane scrollPane = new JScrollPane(table);
        scrollPane.setBorder(BorderFactory.createLineBorder(new Color(230, 230, 230)));
        add(scrollPane, BorderLayout.CENTER);

        refreshTable();
    }

    public void refreshTable() {
        new SwingWorker<List<Purchase>, Void>() {
            @Override
            protected List<Purchase> doInBackground() throws Exception {
                return purchaseService.getAllPurchases();
            }

            @Override
            protected void done() {
                try {
                    List<Purchase> list = get();
                    tableModel.setRowCount(0);
                    for (Purchase p : list) {
                        tableModel.addRow(new Object[]{
                                p.getPurchaseId(),
                                p.getBookTitle(),
                                p.getSupplierName(),
                                p.getQuantity(),
                                "₹" + p.getPricePerBook(),
                                "₹" + p.getTotalAmount(),
                                p.getPurchaseDate()
                        });
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
        }.execute();
    }

    private void showPurchaseDialog() {
        Frame parent = (Frame) SwingUtilities.getWindowAncestor(this);
        JDialog dlg = new JDialog(parent, "Process Book Purchase", true);
        dlg.setUndecorated(true);

        JPanel panel = new JPanel() {
            @Override
            protected void paintComponent(Graphics g) {
                Graphics2D g2 = (Graphics2D) g.create();
                g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
                g2.setColor(Color.WHITE);
                g2.fillRoundRect(0, 0, getWidth(), getHeight(), 16, 16);
                g2.setColor(new Color(230, 230, 230));
                g2.drawRoundRect(0, 0, getWidth() - 1, getHeight() - 1, 16, 16);
                g2.dispose();
            }
        };
        panel.setBackground(Color.WHITE);
        panel.setBorder(new EmptyBorder(25, 25, 25, 25));
        panel.setLayout(new GridBagLayout());
        GridBagConstraints gbc = new GridBagConstraints();
        gbc.fill = GridBagConstraints.HORIZONTAL;
        gbc.insets = new Insets(8, 0, 8, 0);
        gbc.gridx = 0;

        JLabel lblHeader = new JLabel("Acquire New Stock");
        lblHeader.setFont(new Font("Segoe UI", Font.BOLD, 18));
        lblHeader.setForeground(new Color(50, 50, 50));
        gbc.gridy = 0;
        panel.add(lblHeader, gbc);

        RoundedTextField txtBookId = new RoundedTextField();
        txtBookId.setPlaceholder("Book ID");
        gbc.gridy = 1;
        panel.add(txtBookId, gbc);

        RoundedTextField txtSupplier = new RoundedTextField();
        txtSupplier.setPlaceholder("Supplier Name");
        gbc.gridy = 2;
        panel.add(txtSupplier, gbc);

        RoundedTextField txtQty = new RoundedTextField();
        txtQty.setPlaceholder("Quantity to Purchase");
        gbc.gridy = 3;
        panel.add(txtQty, gbc);

        RoundedTextField txtPrice = new RoundedTextField();
        txtPrice.setPlaceholder("Unit Price per Book");
        gbc.gridy = 4;
        panel.add(txtPrice, gbc);

        JPanel btnPanel = new JPanel(new FlowLayout(FlowLayout.RIGHT, 10, 0));
        btnPanel.setOpaque(false);

        RoundedButton btnCancel = new RoundedButton("Cancel", new Color(220, 220, 220), new Color(200, 200, 200));
        btnCancel.setForeground(new Color(80, 80, 80));
        btnCancel.setPreferredSize(new Dimension(90, 35));
        btnCancel.addActionListener(e -> dlg.dispose());

        RoundedButton btnSave = new RoundedButton("Execute Purchase", new Color(24, 144, 255), new Color(11, 101, 192));
        btnSave.setPreferredSize(new Dimension(140, 35));
        btnSave.addActionListener(e -> {
            String bookIdStr = txtBookId.getText().trim();
            String supplier = txtSupplier.getText().trim();
            String qtyStr = txtQty.getText().trim();
            String priceStr = txtPrice.getText().trim();

            if (bookIdStr.isEmpty() || supplier.isEmpty() || qtyStr.isEmpty() || priceStr.isEmpty()) {
                ToastNotification.show(parent, "All fields are required", ToastNotification.Type.ERROR);
                return;
            }

            try {
                int bookId = Integer.parseInt(bookIdStr);
                int qty = Integer.parseInt(qtyStr);
                double price = Double.parseDouble(priceStr);

                new SwingWorker<String, Void>() {
                    @Override
                    protected String doInBackground() throws Exception {
                        Book b = bookService.getBookById(bookId);
                        if (b == null) return "Book ID not found in database";

                        Purchase p = new Purchase();
                        p.setBookId(bookId);
                        p.setSupplierName(supplier);
                        p.setQuantity(qty);
                        p.setPricePerBook(price);
                        p.setTotalAmount(qty * price);
                        p.setPurchaseDate(new Date(System.currentTimeMillis()));

                        purchaseService.purchaseBook(p);
                        return "SUCCESS";
                    }

                    @Override
                    protected void done() {
                        try {
                            String res = get();
                            if (res.equals("SUCCESS")) {
                                ToastNotification.show(parent, "Acquisition processed successfully!", ToastNotification.Type.SUCCESS);
                                dlg.dispose();
                                refreshTable();
                            } else {
                                ToastNotification.show(parent, res, ToastNotification.Type.ERROR);
                            }
                        } catch (Exception ex) {
                            ex.printStackTrace();
                        }
                    }
                }.execute();
            } catch (NumberFormatException ex) {
                ToastNotification.show(parent, "Numeric formats required for quantities & prices", ToastNotification.Type.ERROR);
            }
        });

        btnPanel.add(btnCancel);
        btnPanel.add(btnSave);
        gbc.gridy = 5;
        panel.add(btnPanel, gbc);

        dlg.setContentPane(panel);
        dlg.pack();
        dlg.setSize(350, dlg.getHeight());
        dlg.setLocationRelativeTo(parent);
        dlg.setVisible(true);
    }

    private void exportCSV() {
        Frame parent = (Frame) SwingUtilities.getWindowAncestor(this);
        JFileChooser fileChooser = new JFileChooser();
        fileChooser.setDialogTitle("Export Acquisition History");
        fileChooser.setSelectedFile(new File("Purchases_Report.csv"));
        int selection = fileChooser.showSaveDialog(this);
        if (selection == JFileChooser.APPROVE_OPTION) {
            File file = fileChooser.getSelectedFile();
            if (ExcelExporter.exportToCSV(table, file)) {
                ToastNotification.show(parent, "Data exported successfully", ToastNotification.Type.SUCCESS);
            } else {
                ToastNotification.show(parent, "Export failed", ToastNotification.Type.ERROR);
            }
        }
    }
}
